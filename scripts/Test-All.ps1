...
$runs = @(
    ...
    @{
        ...
        # Восемнадцать своих плюс три обкатки переходника: её отчёт печатается внутри и
        # считается ведомостью так же, как у прогона переходника. Проверки настоящие -
        # их делает платформа, а не прогон. Шесть из восемнадцати - про Show-Locks.sql:
        # живую блокировку он видит только здесь, и только здесь есть с чем сличить ответ.
        Checks = 21; WithInstance = $true; Needs = @('LW_DOC_TABLE_NO', 'LW_DOC_FIELD_NO')
