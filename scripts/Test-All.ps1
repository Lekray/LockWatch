...
$runs = @(
    @{
        ...
        Checks = 45; Critical = $true; WithInstance = $true
    ...
    @{
        ...
        # Одиннадцать своих плюс три обкатки переходника: её отчёт печатается внутри и
        # считается ведомостью так же, как у прогона переходника. Проверки настоящие -
        # их делает платформа, а не прогон.
        Checks = 14; WithInstance = $true; Needs = @('LW_DOC_TABLE_NO', 'LW_DOC_FIELD_NO')
