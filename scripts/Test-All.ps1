...
$runs = @(
    ...
    @{
        ...
        # Двадцать одна своя плюс три обкатки переходника: её отчёт печатается внутри и
        # считается ведомостью так же, как у прогона переходника. Проверки настоящие -
        # их делает платформа, а не прогон. Девять из двадцати одной - про Show-Locks.sql:
        # живую блокировку он видит только здесь, и только здесь есть с чем сличить ответ.
        # Из этих девяти три сняты с блокировки БЕЗ отметок - двумя сессиями sqlcmd: пустую
        # клетку учётной записи иначе не измерить, а объяснять себя обязана и она.
        Checks = 24; WithInstance = $true; Needs = @('LW_DOC_TABLE_NO', 'LW_DOC_FIELD_NO')
