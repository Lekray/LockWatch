...
$runs = @(
    ...
    @{
        ...
        # Двадцать три своих плюс три обкатки переходника: её отчёт печатается внутри и
        # считается ведомостью так же, как у прогона переходника. Проверки настоящие -
        # их делает платформа, а не прогон. Десять из двадцати двух - про Show-Locks.sql:
        # живую блокировку он видит только здесь, и только здесь есть с чем сличить ответ.
        # Из этих десяти четыре сняты не с той блокировки, что меряет имя: три - с блокировки
        # БЕЗ отметок, двумя сессиями sqlcmd, и одна с ТИХОЙ базы. Пустую клетку и пустую
        # очередь иначе не измерить, а объяснять себя обязаны и они.
        Checks = 26; WithInstance = $true; Needs = @('LW_DOC_TABLE_NO', 'LW_DOC_FIELD_NO')
