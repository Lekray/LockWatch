...
# Собственный диапазон инструмента - 110230..110249, и лезть в него собранному объекту
# нельзя. Ловилось на себе: собранный переходник встал на номер кодюнита тревоги, импорт
# его перекрыл, а уборка прогона потом удалила - и отказ вылез через два прогона, в
# совершенно другом месте.
if (($ObjectNo -ge 110230) -and ($ObjectNo -le 110249)) {
    Fail "номер $ObjectNo лежит в собственном диапазоне LockWatch (110230..110249) - у собранного объекта должен быть свой, вне его"
}
