...
function Measure-Mode([string]$why) {
    ...
    # разогревочный, а числом становится второй.
...
"@ | Out-Null
    ...
    Check 'включённая подписка не удваивает цену записи' `
        ($onAdapter.Us -le (2 * $noAdapter.Us)) `
        "без подписчика $($noAdapter.Us) мкс, включён $($onAdapter.Us) мкс при потолке $(2 * $noAdapter.Us)"
