...
"@ | Out-Null
    ...
    # Выключатель ставит сам StartWatch, и правка его здесь была лишней. Хуже того: она
    # опасна - правка мимо NAV кладётся в базу, а служба читает строку из кэша, и первая же
    # запись из NAV в ту же строку падает (FINDINGS, раздел 69). Держалось это лишь тем, что
    # выше стоит перезапуск и настройку до правки никто не читал.
    ...
    # Тихая база - то, что человек видит ЧАЩЕ ВСЕГО: съёмку запускают, пока жалоба ещё
    # свежа, а спор уже кончился. Пустая сетка без единого слова читается как сломанный
    # запрос, и первым замечанием скрипт говорит, что спорить просто некому.
    $deadline = (Get-Date).AddSeconds(20)
    while (((Get-Date) -lt $deadline) -and
           ([int](Scalar "SELECT COUNT(*) FROM sys.dm_os_waiting_tasks WHERE wait_type LIKE 'LCK[_]%';") -gt 0)) {
        Start-Sleep -Milliseconds 300
    }
    $quietOut = & sqlcmd -S $Server -d $Database -E -b -l 30 -w 4000 -W -s '|' -h -1 -i $showLocks 2>&1
    Check 'на тихой базе съёмка говорит, что очередь пуста, а не молчит' `
        ([bool]($quietOut | Where-Object { $_ -match 'Ни одна сессия сейчас не ждёт блокировки' })) `
        "строк в ответе $(@($quietOut).Count)"
...
"@
    ...
    # Причина названа ТОЧНАЯ: спорят сессии sqlcmd, и про них верно не «отметки нет», а
    # «не служба NAV» - имени NAV у такого соединения не существует вовсе, и правильный
    # ответ про него стоит рядом, в логине SQL. Скажи скрипт «отметки нет», человек пошёл
    # бы искать переходник, которого этой стороне не нужно.
    Check 'пустую клетку учётной записи съёмка объясняет В НЕЙ, и объясняет точно' `
        (($m[6] -eq '(не служба NAV)') -and ($m[14] -eq $m[6])) `
    ...
    # Разбор стоит ВТОРОЙ ТАБЛИЦЕЙ ответа, а не PRINT-ом: SSMS уносит PRINT на вкладку
    # «Сообщения», и на бою его не прочли вовсе. Проверка смотрит на текст замечания в
    # ответе - уедь он обратно в сообщения, при -h -1 его тут не будет.
    Check 'разбор причины стоит ВТОРОЙ ТАБЛИЦЕЙ ответа, а не в сообщениях' `
        ([bool]($muteOut | Where-Object { $_ -match 'НЕ соединение службы NAV' })) `
    ...
    # ---------- 4б. тот же вопрос, но скриптом ручного разбора ----------
    # Show-Locks.sql обещает в своей шапке «та же дорога, какой ходит сам инструмент, но
    # одной съёмкой и без установки». Обещание это не проверял никто: скрипта не было ни в
    # одном прогоне и ни в ведомости сметы - а показывают его службе безопасности ДО
    # установки, как ответ на вопрос «что именно вы спрашиваете у сервера».
    #
    # Здесь оно становится замером: блокировка живая, обе стороны - сессии NAV, отметки
    # лежат у обеих. Сличается ответ скрипта с ОТМЕТКАМИ, а не с журналом: журнал заполняет
    # тот же инструмент, и сошлись бы две половины одной дороги.
    #
    # Окно узкое и известное: держатель держит полторы минуты, а ждущая сессия NAV
    # отваливается по своему пределу в 10 000 мс. Отметку ждущего прогон ловит в течение
    # трёхсот миллисекунд после появления, и очереди остаётся ещё секунд девять - съёмка
    # укладывается в неё с запасом, но пробует несколько раз, а не один.
    #
    ...
    while ($true) {
        $out = & sqlcmd -S $Server -d $Database -E -b -l 30 -w 4000 -W -s '|' -h -1 -i $showLocks 2>&1
    ...
    while ($s.Count -lt 23) { $s += '' }

    Check 'скрипт ручного разбора ВЫПОЛНЯЕТСЯ, а не лежит в репозитории картинкой' `
        ($shownCode -eq 0) "код возврата sqlcmd $shownCode"
    Check 'съёмка видит ту же живую блокировку и называет держателя учётной записью NAV' `
        (($shownRow -ne '') -and ($holdUser -ne '') -and ($s[6] -eq $holdUser)) `
        "учётная запись держателя по съёмке [$($s[6])] при ожидаемой [$holdUser]"
    # Отметок в этот миг две, и взять чужую - беда, которая уже случалась с самим
    # инструментом 11.09.2026: отметку виновника искали по сеансу ЖЕРТВЫ, и прогон остался
    # зелёным целиком. Различает их только документ: у ждущего в отметке СВОЙ.
    Check 'съёмка прочитала отметку ДЕРЖАТЕЛЯ, а не ждущего' `
        (($s[18] -ne '') -and ($s[18] -eq $holdDoc) -and ($holdDoc -ne $waitDoc)) `
        "документ отметки [$($s[18])] при документе держателя [$holdDoc] и ждущего [$waitDoc]"
    Check 'логин SQL стоит в съёмке рядом с учётной записью и не подменяет её' `
        (($s[7] -ne '') -and ($s[7] -ne $s[6])) `
        "логин держателя [$($s[7])] при учётной записи [$($s[6])]"
...
"@
    ...
    # Две дороги к одному ответу: съёмка снаружи и сторож изнутри. Сходятся они на том, что
    # обе взяли у сервера, - имя объекта SQL и номер ключа NAV. Разойдись они здесь, и
    # обещание «та же дорога» было бы словом.
    $bothRoads = Scalar @"
SELECT TOP 1 [SQL Object Name] + '|' + CONVERT(varchar(11),[NAV Key No_])
FROM $episode WHERE [NAV Table Name] = N'$navTable' ORDER BY [Entry No_] DESC;
"@
    $br = @(($bothRoads -split '\|') | ForEach-Object { $_.Trim() })
    while ($br.Count -lt 2) { $br += '' }
    Check 'съёмка и сторож назвали один и тот же объект SQL' `
        (($s[3] -ne '') -and ($s[3] -eq $br[0])) "съёмка [$($s[3])], журнал [$($br[0])]"
    Check 'и один и тот же номер ключа NAV' `
        (($s[4] -ne '') -and ($s[4] -eq $br[1])) "съёмка [$($s[4])], журнал [$($br[1])]"
