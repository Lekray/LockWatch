...
.DESCRIPTION
    ...
    подмену виновника жертвой по самому имени прогон не может. Различает он её ДОКУМЕНТОМ -
    у держателя и у ждущего разные строки, и в журнал обязан попасть спорный документ, а не
    тот, которым занималась жертва. Это и есть та ошибка, которой стоит бояться: две отметки
    рядом, и перепутать их дороже, чем не найти ни одной.
